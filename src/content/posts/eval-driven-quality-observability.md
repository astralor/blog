...
description: "上一篇文章我提出了 AI 产品中功能与效果的结构性矛盾。Anthropic 最近发布的 Agent 评估方法论，给了这个问题一个系统性的回应。但我觉得它指向的东西比 Eval 本身更大——AI Agent 的质量，需要像基础设施一样被「可观测」。"
...
几天前我写了一篇关于 AI 产品中[功能迭代与效果优化之间结构性矛盾](/posts/dual-helix-feature-vs-quality)的文章。核心观察是：在 Agentic AI 产品里，功能开发的速度被 AI 辅助工具从根本上改变了，但效果优化的速度被问题本质决定了——修改提示词、等模型跑、观察输出、分析问题、再来一轮。这两条线天然跑在不同的速度上，而且功能每迭代一次，效果的问题空间就膨胀一圈。

那篇文章停在了「一些初步的思考」——归因分层、回归测试、稳定窗口、自动化评估。方向有了，但具体怎么做，当时没有展开。

然后 Anthropic 发了一篇文章。

[Demystifying Evals for AI Agents](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents)，万字长文，系统性地阐述了 AI Agent 评估的方法论——从概念定义到分类型实战，从零到一的路线图，再到评估在整个质量保障体系中的位置。

读完之后我意识到，这篇文章回应的恰好就是我上次留下的那个未完成的思考。不是说 Anthropic 在回应我——而是同一个问题，从不同的位置出发，最终指向了同一个方向。
...
上面的分析听起来很漂亮，但如果就此收笔，这篇文章就犯了我上次自己批评的错误——停在理论层面不落地。

实际操作中，Agent 的质量可观测性有几个绕不过去的难题。
